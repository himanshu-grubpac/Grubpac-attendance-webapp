...
const endingDate = parsed.endingDate
  ? new Date(parsed.endingDate)
  : null;

if (joiningDate && joiningDate < COMPANY_START_DATE) {
  const error = new Error(
    'Employee joining date cannot be before the company start date.'
  );
...
if (joiningDate && endingDate && endingDate < joiningDate) {
  const error = new Error(
    'Employee ending date cannot be before the joining date.'
  );
  error.statusCode = 400;
  throw error;
}
